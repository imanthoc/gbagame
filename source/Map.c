...
void place_fire_tiles()
{
    for (u8 i = 0; i < FIRETILE_CNT; ++i)
    {
        ...
        OAM[oam_index].attr0 = OBJ_Y(y) | ATTR0_COLOR_16 | ATTR0_SQUARE;
        OAM[oam_index].attr1 = OBJ_X(x) | ATTR1_SIZE_16;
        OAM[oam_index].attr2 = ATTR2_PALETTE(0) | OBJ_CHAR(FIRE_TILE_INDEX) | ATTR2_PRIORITY(1);
    }
}
...
u8 check_extant_from_fire(u16 pl_x, u8 pl_y)
{
    ...
    {
        u8 oam_index = FIRE_OAM_INDEX + i;
        u16 f_y = OAM[oam_index + i].attr0 & 0xFF;
        u16 f_x = OAM[oam_index + i].attr1 & 0x1FF;
...
void advance_fire_anim()
{
    for (u8 i = 0; i < FIRETILE_CNT; ++i)
    {
        ...
        {
            u8 cur_tile = OAM[oam_index].attr2 & 0x03FF;
            u8 next_tile;
            ...
            OAM[oam_index].attr2 = ATTR2_PALETTE(0) | OBJ_CHAR(next_tile) | ATTR2_PRIORITY(1);
...
u8 map_scroll_right()
{
    ...
    else
    {
        ...
        {
            u8 oam_index = FIRE_OAM_INDEX + i;
            u16 cur_x = OAM[oam_index].attr1 & 0x1FF;
            OAM[oam_index].attr1 = OBJ_X(cur_x-2) | ATTR1_SIZE_16;
        }
